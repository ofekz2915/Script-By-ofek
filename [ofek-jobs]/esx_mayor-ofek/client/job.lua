...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(1)
        if true then
            if ESX.PlayerData.job.name == "mayor" then
                        ...
                        if true then
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    ...
                                    TriggerServerEvent('test:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Open Stash")
                                end
                                 ...
                                 if IsControlJustReleased(0, Keys["E"]) then
                                     if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                        local veh = GetVehiclePedIsIn(GetPlayerPed(-1))
                                         ...
                                         TriggerServerEvent('test:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Stored a vehicle")
                                     else
                                         MenuGarage()
                                         currentGarage = k
                        ...
                        if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 4.5) and ESX.PlayerData.job.grade_name == 'boss' then
                             if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    ...
                                    TriggerServerEvent('unicorn:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Open unicorn Menu")
                                end
                            elseif (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2.5) then
...
function MenuGarage()
    ...
    Menu.addButton("Close Menu", "closeMenuFull", nil) 
    TriggerServerEvent('test:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Opened garage menu")
end
...
function TakeOutVehicle(vehicleInfo)
    ...
    ESX.Game.SpawnVehicle(vehicleInfo, GetEntityCoords(PlayerPedId()), GetEntityHeading(PlayerPedId()), function(vehicle)
        ...
        local VehName = GetDisplayNameFromVehicleModel(hashVehicule)
        TriggerServerEvent('test:logs', GetPlayerName(PlayerId()) .. '(' .. GetPlayerServerId(PlayerId()) .. ') ' .. 'Taked out "' .. VehName .. '"')
